...
class ManualGCNLayer():
    def __init__(self, in_dim, out_dim, apply_relu = True):
        self.X = None
    ...
    def forward(self, X, adjm):
        ...
        y = np.dot(np.dot(adjm, X), self.weights)
        if self.apply_relu:
    ...
    def backward(self, Y_grad, adjm, learningRate):
        # If relu was applied, mask the gradient
        if self.apply_relu and self.mask is not None:
            Y_grad = Y_grad * self.mask
...
class ManualGCN():
    def __init__(
        ...
        # Layers
        self.gcn1 = ManualGCNLayer(input_dim, hidden_dim, True)
        self.dropout = ManualDropLayer(p=dropout_p)
        self.gcn2 = ManualGCNLayer(hidden_dim, output_dim, False)
