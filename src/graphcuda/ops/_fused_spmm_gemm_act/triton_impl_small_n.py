...
@triton_op("graphcuda::fused_spmm_gemm_relu_small_n", mutates_args={})
def fused_spmm_gemm_relu_small_n(
    M: int,
    K1: int,
    BLOCK_M: int,
    adjm_bsr_values_rm: torch.Tensor, # BSR values can't be passed as a sparse tensor because not supported by triton op
    adjm_bsr_crow_indices: torch.Tensor,
    adjm_bsr_col_indices: torch.Tensor,
    X: torch.Tensor,
    ...
    assert adjm_bsr_values_rm.dim() == 1, "adjm_bsr_values_rm must be 1D"
    assert adjm_bsr_crow_indices.dim() == 1, "adjm_bsr_crow_indices must be 1D"
    assert adjm_bsr_col_indices.dim() == 1, "adjm_bsr_col_indices must be 1D"
    ...
    # c. shapes
    K2, N = weights.shape[0], weights.shape[1]
    ...
    # d. devices
    device = X.device
    assert X.device.type == "cuda", "adjm must be on CUDA"
    assert adjm_bsr_values_rm.device == device, "adjm_bsr_values_rm must be on the same device as X"
    assert adjm_bsr_crow_indices.device == device, "adjm_bsr_crow_indices must be on the same device as X"
    assert adjm_bsr_col_indices.device == device, "adjm_bsr_col_indices must be on the same device as X"
    assert weights.device == device, "weights must be on the same device as X"
    
    # e. dtypes
    dtype = X.dtype
    assert X.dtype == weights.dtype, "X and weights must have the same dtype"
    assert adjm_bsr_values_rm.dtype == dtype, "adjm_bsr_values_rm must have the same dtype as X"
    ...
    if has_bias:
        ...
        assert bias.device == device, "bias must be on the same device as X"
    ...
    assert adjm_bsr_crow_indices.numel() == nrow_blocks + 1, "unexpected BSR crow_indices length"
    ...
    wrap_triton(fused_spmm_gemm_relu_small_n_kernel)[grid](
        M, K1, K2, N,
        X, weights, Y,
        relu_mask if relu_mask is not None else 0,
        adjm_bsr_values_rm, adjm_bsr_crow_indices, adjm_bsr_col_indices,
